...
func testUserToManyMeetups(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, meetupDBTypes, false, meetupColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, meetupDBTypes, false, meetupColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Meetups().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadMeetups(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Meetups); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Meetups = nil
	if err = a.L.LoadMeetups(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Meetups); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpMeetups(t *testing.T) {
	var err error
	...
	foreigners := []*Meetup{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, meetupDBTypes, false, strmangle.SetComplement(meetupPrimaryKeyColumns, meetupColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Meetup{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddMeetups(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Meetups[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Meetups[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Meetups().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `Email`: `character varying`, `Username`: `character varying`}
	_           = bytes.MinRead
)
